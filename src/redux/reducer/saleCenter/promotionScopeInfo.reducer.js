...
} from '../../actions/saleCenter/promotionScopeInfo.action.js';
...
function getDataStructureContainCityAndArea(data) {
    ...
    let _compositeData = cities.map((city)=>{
        let _children =  areas
            .filter((area) => {
                return area.cityID == city.cityID;
            })
            .map((area)=>{
                let _children = shops.filter((shop)=>{
...
export const  promotionScopeInfo = ($$state = $initialState, action) => {
    switch (action.type) {
        ...
        case SALE_CENTER_FETCH_PROMOTION_SCOPE_INFO_SUCCESS:
            let constructedData = constructTreeDataContainsCityAndShop(action.payload);
            let cityAreasShops = getDataStructureContainCityAndArea(action.payload);
            return $$state
