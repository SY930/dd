...
export const arrayTransformAdapter = function (source) {
    if (!(source instanceof String || typeof source === "string")) {
        throw new Error(
            `The source string should be string, which is ${source}`
        );
    }
...
export const promotionBasicDataAdapter = function (source, dir) {
    ...
    if (!dir) {
        ...
        if (
            _source.master.startDate === 20000101 ||
            _source.master.startDate === 29991231
        ) {
        ...
        const _excludeDateArray = arrayTransformAdapter(
            _source.master.excludedDate
        ).map((ed) => {
    ...
    const validCycle = ((validType) => {
        if (validType == "1") {
            return _source.selectWeekValue
                .map((week) => {
                    return `w${week}`;
                })
                .join(",");
        } else if (validType == "2") {
            return _source.selectMonthValue
                .map((month) => {
                    return `m${month}`;
                })
                .join(",");
...
export const promotionDetailInfoAdapter = function (source, dir) {
    ...
    if (!dir) {
        ...
        try {
            ruleJson =
                source.master.ruleJson != "" &&
                JSON.parse(source.master.ruleJson);
        } catch (err) {
            ruleJson = {};
        }
        return {
    ...
    if (
        source.scopeLst &&
        source.scopeLst.length > 0 &&
        source.foodCategory.length == 0 &&
        source.excludeDishes.length == 0 &&
...
export const getPromotionIdx = function (promotionKey) {
    ...
    const index = [...ACTIVITY_CATEGORIES, ...ONLINE_PROMOTION_TYPES].findIndex(
        (promotionInfo) => promotionInfo.key === promotionKey
    );

    if (index >= 0) return index;
    throw new Error(
        `There is not promotion with the specified promotionKey ${promotionKey}`
    );
};

export const getSpecialPromotionIdx = function (promotionKey) {
    const promotionKeyStr = String(promotionKey);
    const _promotionInfo = CHARACTERISTIC_CATEGORIES.find(
        (promotionInfo) => promotionInfo.key === promotionKeyStr
    );
    if (_promotionInfo) {
