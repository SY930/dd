 ...
 * @LastEditTime: 2022-10-11 10:33:47
...
class SeckillInLimitedTime extends Component {
    ...
    transformFormData = res => {
        ...
        let formData = {
            ...
            gifts: gifts.map(item => {
                return {
                    ...
                    rangeDate:
                        item.effectType == 2 && item.effectTime && item.validUntilDate
                            ? [moment(item.effectTime, "YYYYMMDDHHmmss"), moment(item.validUntilDate, "YYYYMMDDHHmmss")]
                            : []
                };
            }),
            timeList: timeList.length
                ...
                : [{ id: "0" }], //时间段增加默认值
            partInTimes1: data.countCycleDays != "0" ? data.partInTimes : undefined,
            joinType: data.countCycleDays != "0" ? "2" : data.partInTimes != "0" ? "1" : "0"
        };
        ...
        //高级日期设置 true/false
        let advMore = false;
        if (timeList.length || cycleType || (data.excludedDate && data.excludedDate.length)) {
            advMore = true;
        }
    ...
    checkAndFormatParams = values => {
        const { validCycle = [], cycleType, eventRange = [], gifts = [], timeList = [] } = values;
        ...
        const event = {
            ...
            partInTimes: values.countCycleDays && values.countCycleDays != 0 ? values.partInTimes1 : values.partInTimes,
            countCycleDays: values.countCycleDays,
            autoRegister: values.autoRegister != undefined ? values.autoRegister : 1
        };
        let params = {
            event,
            gifts: [],
        ...
        if (values.couponType == 1) {
            ...
            params.gifts = this.state.slectedWxCouponList.map(item => {
                return {
                    ...
                    giftEffectTimeHours: item.effectGiftTimeHours || item.giftEffectTimeHours,
                    sendType: "0",
                    giftCount: values.giftCount,
                    presentType: 7,
                    giftValidUntilDayCount: item.validUntilDays || item.giftValidUntilDayCount,
            ...
            params.gifts = gifts.map(item => {
                ...
                let v = {
                    ...item,
                    effectType: item.effectType == 2 ? 2 : item.countType == "0" ? 1 : 3,
                    effectTime: item.effectType == 2 && item.rangeDate && item.rangeDate.length ? item.rangeDate[0].format("YYYYMMDD") : "",
                    validUntilDate: item.effectType == 2 && item.rangeDate && item.rangeDate.length ? item.rangeDate[1].format("YYYYMMDD") : ""
                };
                delete v.countType;
                return v;
            });
    ...
    handleSubmit = () => {
        const { basicForm, ruleForm, giftsForm = [], slectedWxCouponList = [] } = this.state;
        ...
        axios
            ...
            .then(res => {
                if (res.code === "000") {
                    ...
                    this.setState({
                        paramsValue: eventParamInfoList.find(item => item.eventWay == 23).paramValue
                    });
                } else {
        ...
        return (
            <div className={styles.formContainer}>
                ...
                <Spin spinning={loading}>
                    ...
                    <BasicInfoForm basicForm={basicForm} getForm={form => this.setState({ basicForm: form })} formData={formData} {...itemProps} />
