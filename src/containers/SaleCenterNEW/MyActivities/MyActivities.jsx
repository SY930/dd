...
const mapStateToProps = (state) => {
    return {
        ...
        promotionList: state.sale_promotionAutoRunState
            .get("promotionList")
            .toJS(),
...
class MyActivities extends React.Component {
    constructor(props) {
        ...
        this.renderModifyRecordInfoModal = this.renderModifyRecordInfoModal.bind(
            this
        );
        this.onDateQualificationChange = this.onDateQualificationChange.bind(
            this
        );
        this.renderShopsInTreeSelectMode = this.renderShopsInTreeSelectMode.bind(
            this
        );
    ...
    fromOnsaleJump = () => {
        ...
        console.log(this.getQueryVariable(), "this.getQueryVariable()1");
        if (from === "onSale") {
            this.setState(
                {
    ...
    onWindowResize = () => {
        ...
        if (parentDoms != null) {
            ...
            if (undefined != contentrDoms && contentrDoms.length > 0) {
                ...
                const headerDoms = parentDoms.querySelectorAll(
                    ".layoutsHeader"
                );
                const headerHeight = headerDoms[0].getBoundingClientRect()
                    .height;
                layoutsContent.style.height = `${
                    parentHeight - headerHeight - 200
                }px`; // layoutsContent 的高度，等于父节点的高度-头部-横线-padding值
                this.setState({
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (
            this.props.myActivities.get("$promotionList") !=
            nextProps.myActivities.get("$promotionList")
        ) {
            const _promoitonList = nextProps.myActivities
                .get("$promotionList")
                .toJS();
    ...
    getParams = () => {
        ...
        if (
            ...
            promotionDateRange[0] !== undefined
        ) {
    ...
    changeSortOrder(record, direction) {
        ...
        axiosData(
            "/promotion/docPromotionService_updateRanking.ajax",
            params,
            { needThrow: true },
            { path: undefined },
            "HTTP_SERVICE_URL_PROMOTION_NEW"
    ...
    successFn = (responseJSON) => {
        const _promotionIdx = getPromotionIdx(
            `${this.state.editPromotionType}`
        );
        const _serverToRedux = false;
        if (
            responseJSON.promotionInfo === undefined ||
            responseJSON.promotionInfo.master === undefined
        ) {
        ...
        if (responseJSON.promotionInfo.master.maintenanceLevel === "1") {
            ...
            this.props.fetchFoodCategoryInfo(
                { ...opts },
                isHuaTian(),
                responseJSON.promotionInfo.master.subGroupID
            );
        ...
        this.props.saleCenterResetBasicInfo(
            promotionBasicDataAdapter(
                responseJSON.promotionInfo,
                _serverToRedux
            )
        );
        this.props.saleCenterResetScopeInfo(
            promotionScopeInfoAdapter(
                responseJSON.promotionInfo.master,
                _serverToRedux
            )
        );
        this.props.saleCenterResetDetailInfo(
    ...
    checkDetailInfo() {
        ...
        this.props.fetchPromotionDetail_NEW({
            data: {
                promotionID: _record
                    ? _record.promotionIDStr
                    : this.state.currentPromotionID, // promotionID 会自动转换int类型,出现数据溢出,新加字符串类型的promotionIDStr替换
                groupID: this.props.user.accountInfo.groupID,
            },
            fail: this.failFn,
        });
    ...
    renderContentOfThisModal() {
        const promotionDetailInfo = this.props.myActivities
            .get("$promotionDetailInfo")
            .toJS();
        const _state = this.state;
        if (
            promotionDetailInfo.status === "start" ||
            promotionDetailInfo.status === "pending"
        ) {
            return (
        ...
        if (
            promotionDetailInfo.status === "timeout" ||
            promotionDetailInfo.status === "fail"
        ) {
            return (
                <div className={styles.spinFather}>
                    {SALE_LABEL.k5doax7i}{" "}
    ...
    parseResponseJson = (rsp, successCode) => {
        const resultcode =
            rsp.resultcode === undefined ? rsp.code : rsp.resultcode;
        const resultmsg =
            rsp.resultmsg === undefined
                ? rsp.msg === undefined
                    ? rsp.message
                    : rsp.msg
                : rsp.resultmsg;
        const isSuccess =
            rsp.success !== undefined
                ? rsp.success
                : resultcode === successCode;
        const doRelogin =
            resultcode === "0011111100000001" ||
        ...
        return {
            ...
            msg: doRelogin
                ? "会话失效,请重新登录"
                : resultmsg || rsp.statusText || "网络错误，请稍后重试",
            redirect: doRelogin,
        };
    };
    handleOperateSave = () => {
            ...
            >
                <div>
                    <div style={{ marginTop: 13, marginLeft: 20 }}>
                        ...
                        <RadioGroup
                            onChange={this.handleExecuteTimeType}
                            value={this.state.executeTimeType}
                        >
                            <Radio key={"1"} value={1}>
                                按开台时间计算
                            </Radio>
                    ...
                    <div style={{ marginTop: 13, marginLeft: 20 }}>
                        ...
                        <RadioGroup
                            ...
                            value={this.state.executeFoodUnitType}
                        >
                            <Radio key={"0"} value={0}>
                                按主规格计算
                            </Radio>
    ...
    renderShopsInTreeSelectMode() {
        const treeData = Immutable.List.isList(
            this.props.promotionScopeInfo.getIn([
                "refs",
                "data",
                "constructedData",
            ])
        )
            ? this.props.promotionScopeInfo
                  .getIn(["refs", "data", "constructedData"])
                  .toJS()
            : this.props.promotionScopeInfo.getIn([
                  "refs",
                  "data",
                ...
                <div className="layoutsSearch">
                    <ul>
                            ...
                            <Select
                                ...
                                filterOption={(value, option) =>
                                    typeof option.props.children == "string"
                                        ? option.props.children.indexOf(value) >
                                          -1
                                        : option.props.children.props.defaultMessage.indexOf(
                                              value
                                          ) > -1
                                }
                                placeholder=""
                        ...
                        <li>
                            ...
                            >
                                <Option value={TRIPLE_STATE.ALL}>
                                    {k5eng042}
                                </Option>
                                <Option value={TRIPLE_STATE.OPTION1}>
                                    {COMMON_LABEL.enable}
                                </Option>
                                <Option value={TRIPLE_STATE.OPTION2}>
                                    {COMMON_LABEL.disable}
                                </Option>
                            </Select>
                        </li>
                        ...
                        <li>
                            <Authority
                                rightCode={BASIC_PROMOTION_QUERY}
                                entryId={BASIC_PROMOTION_MANAGE_PAGE}
                            >
                                <Button
                                    type="primary"
    ...
    renderAdvancedFilter() {
        let categories = [],
            tags = [],
            brands = [];
        const $categories = this.props.promotionBasicInfo.getIn([
            "$categoryList",
            "data",
        ]);
        ...
        const $brands = this.props.promotionScopeInfo.getIn([
            "refs",
            "data",
            "brands",
        ]);
                    ...
                    <ul>
                        ...
                        <li>
                            <Select
                                ...
                                placeholder=""
                            >
                                {SALE_CENTER_ACTIVITY_ORDER_TYPE_LIST.map(
                                    (order) => {
                                        return (
                                            <Option
                                                key={`${order.value}`}
                        ...
                        <li>
                            <Select
                                ...
                                placeholder=""
                            >
                                {SALE_CENTER_ACTIVITY_SUITSENCE_LIST.map(
                                    (order) => {
                                        return (
                                            <Option
                                                key={`${order.value}`}
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    ...
                    return (
                        <span>
                            <Authority
                                rightCode={BASIC_LOOK_PROMOTION_QUERY}
                                entryId={BASIC_PROMOTION_MANAGE_PAGE}
                            >
                                <a
                                    href="#"
                            ...
                            {!isHuaTian() && (
                                <Authority
                                    rightCode={BASIC_PROMOTION_UPDATE}
                                    entryId={BASIC_PROMOTION_MANAGE_PAGE}
                                >
                                    <a
                                        href="#"
                            ...
                            <Authority
                                rightCode={BASIC_PROMOTION_DELETE}
                                entryId={BASIC_PROMOTION_MANAGE_PAGE}
                            >
                                {/* 非禁用状态不能删除 */}
                                <a
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    const defaultChecked =
                        record.isActive == "1" ? true : false; // 开启 / 禁用
            ...
            // },
            {
                ...
                render: (promotionType) => {
                    const promotion = this.getAllPromotionTypes().filter(
                        (promotion) => {
...

            {
                ...
                render: (validDate) => {
                    ...
                    const text = `${moment(String(validDate.start)).format(
                        "YYYY.MM.DD"
                    )} / ${moment(String(validDate.end)).format("YYYY.MM.DD")}`;
                    return text;
                },
            },
            {
            ...
            },
            {
                ...
                render: (status) => {
                    return status == "1" ? (
                        <span className={styles.unBegin}>{k5dlp2gl}</span>
                    ) : status == "2" ? (
                        <span className={styles.begin}>{k5dlp7zc}</span>
                    ) : (
                        <span className={styles.end}>{k5dlpczr}</span>
                    );
                },
            },
            {
                ...
                render: (text, record, index) => {
                    ...
                    return `${record.createBy}/${
                        record.modifiedBy || record.createBy
                    }`;
                },
            },

            {
                ...
                render: (text, record, index) => {
                    ...
                    const t = `${moment(
                        new Date(parseInt(record.createTime))
                    ).format("YYYY-MM-DD HH:mm:ss")} / ${moment(
                        new Date(parseInt(record.actionTime))
                ...
                <Table
                    ...
                    pagination={{
                        ...
                        showTotal: (total, range) =>
                            `${k5ey8jvj}${range[0]}-${range[1]} / ${k5ey8l0e} ${total} ${k5ey8lip}`,
    ...
    render() {
        ...
        return (
            ...
            >
                <div>
                    {this.props.tabKeys !== "saleSpecialPage" &&
                        this.renderHeader()}
                </div>
