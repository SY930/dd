    ...
    mapPriceScopeLstToDataThenEmit = () => { // 只触发一次
        ...
        priceLst.forEach((price) => {
            ...
            const matchedDish = dishes
                .find(dish => dish.value === `${price.foodName}${price.foodUnitName}`);
            if (matchedDish) {
        ...
        scopeLst.forEach((scope) => {
            ...
            const matchedDish = dishes
                .find(dish => dish.value === `${scope.targetName}${scope.targetUnitName}`);
            if (matchedDish) {
...
export default connect(mapStateToProps)(CollocationTableWithoutBrandID);
