...
class SendCard extends React.Component {
        ...
        this.state = {
            ...
            columns: [
                ...[{
                    ...
                    dataIndex: 'operate',
                    key: 'operate',
                    render: (v, rec)=><span className="operate"><a href="javaScript:;"
                                                                   onClick={()=>this.handleMore(rec)}>详情</a></span>
                }],
                ...SENDCARD_COLUMNS
            ],
    ...
    componentDidMount() {
        ...
        const {_key, batchNO, quotaList} = this.props;
        this.setState({
            formData: {batchNO_madeCard: batchNO},
        });
        if (_key == 'sum') {
            const {shopData, FetchGiftSchema} = this.props;
            const _shopData = shopData.toJS();
            if (_shopData.length > 0) {
                this.proShemaData(_shopData);
            } else {
                FetchGiftSchema({}).then((shops = []) => {
                    this.proShemaData(shops);
                });
            }
        }
    ...
    componentWillReceiveProps(nextProps) {
        ...
        const {batchNO, quotaList, _key} = nextProps;
        this.setState({
            formData: {batchNO_madeCard: batchNO}
        });
        this.proRecords(quotaList, _key);
    ...
    getData(_params = {}) {
        const {data:{giftItemID}, _key, FetchQuotaList} = this.props;
        ...
        //     let records = [];
        //     if (_key == 'send') {
        //         records = data.quotaCardList || [];
        //     } else {
        //         records = data.quotaBatchDetails || [];
        //
        //     }
    ...
    proParamsByType = params => {
        ...
        switch (_key) {
            ...
            case 'sum':
                ...
                params = _.omit(params, 'cardNO_sum', 'cardNO_sum', 'timeRangeSend_sum');
                return params;
        }
    }
    ...
    handleMore(rec) {
        ...
        const {UpdateTabKey, UpdateBatchNO} = this.props;
        UpdateTabKey({
            key: 'made',
        });
        UpdateBatchNO({
    ...
    handleCancel(reload) {
        this.reloading((loading)=> {
            ...
            const {FetchQuotaCardSum, data} = this.props;
    ...
    render() {
        const {loading, dataSource, cardProps, formData, sumData} = this.state;
        const {_key} = this.props;
            ...
            <div className={styles.cardSummarize}>
                            ...
                            _key == 'made' ?
                                <Row>
                                    ...
                                    <Col span={8}><Button type="ghost" onClick={()=>this.handleDelete()}><Iconlist
                                        className="send-gray" iconName={'作废'}/>作废</Button></Col>
                                    <Col span={8}><Button type="ghost"
                                                          onClick={()=>this.handleCancelDelete()}>取消作废</Button></Col>
                                </Row>
                                : (
                                _key == 'send' ?
                                    <Row>
                                        ...
                                        <Col span={8}><Button type="ghost" onClick={()=>this.handleSend()}><Iconlist
                                            className="send-gray" iconName={'发布'}/>发卡</Button></Col>
                                    </Row>
                                    :
                                    <Row>
                                        <Col span={24} style={{textAlign: 'right'}}><Button type="primary"
                                                                                            onClick={()=>this.handleQuery()}><Icon
                                            type="search"/>查询</Button></Col>
                                    </Row>
                            )
                ...
                <Row>
                    <Table
                        ...
                        scroll={_key == 'made' ? {} : ( _key == 'sum' ? {x: 1300} : {x: 900})}
...
function mapStateToProps(state) {
    return {
        ...
        batchNO: state.sale_old_giftInfo.get('batchNO'),
        shopData: state.sale_old_giftInfo.get('shopData'),
        quotaList: state.sale_old_giftInfo.get('quotaList'),
    }
}
function mapDispatchToProps(dispatch) {
    return {
        ...
        FetchGiftSchema: opts => dispatch(FetchGiftSchema(opts)),
        FetchQuotaList: opts => dispatch(FetchQuotaList(opts)),
    };
}
