...
import {fetchSpecialUserList} from '../../../redux/actions/saleCenter/mySpecialActivities.action';
import {CHARACTERISTIC_CATEGORIES, SEND_MSG} from '../../../redux/actions/saleCenter/types';
...
class SpecialPromotionDetail extends React.Component {
    ...
    // 基本信息
    renderBaseInfo(){
        let record = this.state.eventInfo.data;
    ...
    renderGiftInfoTable(){
        ...
        let record = this.state.eventInfo.gifts || [];
        let dataSource =  record.map((gift, index)=>{
            return{
                key: `${index}`,
                idx: `${index}`,
                EGiftName: gift.giftName,
                EGiftSingleCount: gift.giftCount,
                EGiftSendCount:gift.giftSendCount,
                EGfitValidUntilDayCount:gift.giftValidUntilDayCount
            }
        });
        return (
