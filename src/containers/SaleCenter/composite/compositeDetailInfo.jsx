...
import PromotionDetailSettings from '../../../containers/SaleCenter/common/promotionDetailSettings';
...
class CompositeDetailInfo extends React.Component{
    ...
    componentWillReceiveProps(nextProps){
        if(nextProps.promotionDetailInfo.getIn(['$promotionDetail','scopeLst']) != this.props.promotionDetailInfo.getIn(['$promotionDetail','scopeLst'])){
            let _scopeLst = nextProps.promotionDetailInfo.getIn(['$promotionDetail','scopeLst']).toJS();
            this.setState({
                scopeLst: _scopeLst,
            },()=>{
                this.sortScopeLst(this.state.scopeLst);
    ...
    handleSubmit(){
        let nextFlag = true;
        let {data, conditions} = this.state;
        let scopeLst = [],
            rule = {
                stageType: 2,
                stage:[
        ...
        let countStr = conditions.map((condition)=>{
    ...
    renderPromotionSetting(){
        return (
            this.state.data.map((item, idx)=>{
                ...
                return (
                    <FormItem key={`group${idx}`} label={`条件${idx+1}`} className={[styles.FormItemStyle,styles.inputWrappers].join(' ')}
                              wrapperCol={{span: 17}} labelCol={{span: 4}}  validateStatus={item.validateStatus}
    ...
    renderConditions(){
        let options = this.state.data.map((dataItem, dataIndex)=>{
            return {
                    ...
                    <div key={`${idx}`} className={styles.moreFormItem}>
                        <Col span={10} offset={3}>
                            <li className={checkStyle.checkBoxStyles}>
                                <FormItem>
                                    <CheckboxGroup value={item.groupCount} options={options} onChange={(val)=>{
                        ...
                        <FormItem className={styles.radioInLine}>
                            ...
                            <FormItem validateStatus={item.flag == '0'?item.cutStatus:item.discountStatus}>
                                {
                                    item.flag == '0'?
                                        <PriceInput
                                            addonAfter="元"
    ...
    renderGroupIcon(idx){
        if(idx == 1 && this.state.data.length == 2){
            ...
            return (
                <span className={styles.iconsLeftStyle}>
                    <Icon className={styles.pulsIcon} disabled={false} type="plus-circle-o" onClick={this.addGroup}/>
                    <Icon className={styles.deleteIcon} type="minus-circle-o" onClick={() => {
