...
class SpecialDishesTable extends React.Component {
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(
            ...
            let priceLst = nextProps.promotionDetailInfo.getIn(['$promotionDetail','priceLst']).toJS();
            this.setState({
                priceLst
            }, ()=>{
                this.sortData(this.state.foodCategoryCollection, this.state.priceLst)
    ...
    handleFoodGroupSelect(value){
        if (value instanceof Array) {
            // get the selections
            let {foodSelections, foodOptions, priceLst }= this.state;
    ...
    clear(){
        ...
        for (let food in foodSelections) {
            food.newPrice = food.price;
        }
    ...
    handleOk = () =>{
        this.handleCancel();
        let data = Array.from(this.state.foodSelections);
    ...
    render() {
        ...
        return (
            <FormItem className={styles.FormItemStyle}>
                ...
                <Row>
                    <Col>
                        <Table bordered dataSource={data} columns={columns} scroll={{ x: 660 }} pagination={{size:'small',pageSize:10}}/>
                    </Col>
                </Row>
