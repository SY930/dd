...
class AdvancedTimeSetting extends React.Component {
    ...
    _remove(index){
        ...
        if (1 === this.state.timeSlot.length) {
            return null;
        }
    ...
    _add(){
        ...
        let length = this.state.timeSlot.length;
    ...
    _onTimePickerChange(time, startOrEnd, index){
        ...
        let _validate = this.state.timeSlot.reduce((preStatus, timeArr)=>{
            let _status = timeArr.start.isBefore(timeArr.end);
