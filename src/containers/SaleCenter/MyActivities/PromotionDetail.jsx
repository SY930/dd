...
class PromotionDetail extends React.Component {
    ...
    renderChannelLst(channelLst){
        //
        let temp =  SALE_CENTER_ACTIVITY_CHANNEL_LIST.filter((channel) => {
    ...
    renderOrderTypeLst(orderTypeLst){
        ...
        try {
            let orderList = arrayTransformAdapter(orderTypeLst);
    ...
    renderActivityRangeInfo(){
        let record = this.props.record;
        let $promotionScopeInfo = this.props.promotionScopeInfo.getIn(["refs", "data", "brands"]);
        return (
            <div>
