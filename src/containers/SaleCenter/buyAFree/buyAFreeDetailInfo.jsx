...
class BuyAFreeDetailInfo extends React.Component{
    ...
    handleSubmit = () => {
        ...
        data = data.map((rule, idx)=>{
            if(rule.stageAmount == null || rule.stageAmount==''){
                rule.stageAmountFlag = false;
                nextFlag = false;
            }
            if(rule.freeAmount == null || rule.freeAmount==''){
    ...
    onStageAmountChange(value, idx){
        let {data} = this.state;
        let stageAmount, stageAmountFlag;
    ...
    ruleTypeChange(val){
        ...
        this.setState({ruleType: val, data: data})
    }

    renderOperationIcon(index){
        let _len = this.state.data.length;
            ...
            return (
                <Row key={`row${idx}`}>
                    <Col span={17} offset={4}>
                        <Col span={14}>
                            <FormItem className={[styles.selectInInput,styles.FormItemStyle,styles.explainBack].join(' ')} wrapperCol={{span:24}}
                                      validateStatus={rule.stageAmountFlag?'success':'error'}
                                      help={rule.stageAmountFlag?null:'请输入购买份数'}>
                        ...
                        <Col span={9} offset={1}>
                            <FormItem
                                className={styles.FormItemStyle} wrapperCol={{span:24}}
                                validateStatus={rule.freeAmountFlag?'success':'error'}
                                help={rule.freeAmountFlag?null:'请输入减免份数'}>

                                <PriceInput addonBefore={'免'}
                                            addonAfter={'份最低价菜品'}
