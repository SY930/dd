...
import { fetchSpecialDetailAC } from "../../redux/actions/saleCenterNEW/mySpecialActivities.action";
import {
...
import { saleCenterResetBasicInfoAC, saleCenterSetBasicInfoAC } from "../../redux/actions/saleCenterNEW/promotionBasicInfo.action";
...
class NewCustomerPage extends Component {
    ...
    handleNewPromotionCardClick(promotionEntity = {}, ifskip) {
        ...
        if (isSpecial) {
            const specialIndex = this.props.saleCenter
    ...
    render() {
        ...
        let ALL_PROMOTION_CATEGORIES = [
            {
                title: "人气活动",
                list: NEW_CUSTOMER_PROMOTION_TYPES.concat(
                    FANS_INTERACTIVITY_PROMOTION_TYPES,
                    REPEAT_PROMOTION_TYPES,
                    LOYALTY_PROMOTION_TYPES,
                    SALE_PROMOTION_TYPES,
                    ONLINE_PROMOTION_TYPES
                ).filter(item => item.isNew && item.key != 67 && item.key != 68)
            },
            {
                title: k6316hto, //  会员拉新
        ...
        ALL_PROMOTION_CATEGORIES = ALL_PROMOTION_CATEGORIES.map(item => ({ ...item, list: _.sortBy(item.list, o => o.isNew) }));
                        ...
                        <div className={selfStyle.platformArea}>
                            ...
                            <div className={selfStyle.platformBox}>
                                {[
                                    ...
                                    <div
                                        onClick={() => {
                                            this.setState({ currentPlatformIndex: index, currentPlatformName: item.name });
                                        }}
                                        className={`${selfStyle.platformItem} ${index === currentPlatformIndex ? selfStyle.selectedItem : ""} ${
                                            index === currentPlatformIndex + 1 ? selfStyle.removeLine : ""
                                        }`}
                                    >
                                        <span>{item.name}</span>
                                    </div>
