    ...
    render() {
        ...
        return (
            <div className={styles.ProgressBar}>
                ...
                <div className={styles.stepsContent} style={{ marginTop: '-15px' }}>{steps[this.state.current].content}</div>
                <div className={styles.stepsAction}>
                    <Button type="ghost" onClick={() => this.cancel('cancel')}>取消</Button>
                    {
                        this.state.current > 0 && <Button style={{ marginLeft: 8 }} type="primary" onClick={() => this.prev('prev')}> 上一步 </Button>
                    }
                    {
                        this.state.current < steps.length - 1 && <Button style={{ marginLeft: 8 }} type="primary" onClick={() => this.next('next')} htmlType="submit">下一步</Button>
                    }
                    {
                        this.state.current === steps.length - 1 && <Button
                            style={{ marginLeft: 8 }}
                            type="primary"
                            onClick={() => {
                                this.cancel('finish');
                                /* message.success('活动新建成功！') */
