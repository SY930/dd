...
class ExpireDateNotice extends React.Component {
        ...
        axios
            ...
            .then((ret) => {
                if (ret.code === "000") {
                    ...
                    if (productCode === "HLL_CRM_Marketingbox") {
                        ...
                        basicAuthEndDate =
                            plugins && plugins.length > 0
                                ? plugins[0].authEndDate
                                : null;
                    } else {
                        basicAuthEndDate = ret.data.basicAuthEndDate
                            ? ret.data.basicAuthEndDate
                            : null;
                    }
                    let endDate = "";
                    let showDate = "";
                    ...
                    if (basicAuthEndDate) {
                        endDate = String(basicAuthEndDate)
                            .match(/(\d{4})(\d{2})(\d{2})/)
                            .filter((item, index) => index > 0)
                            .join("/");
                        showDate = String(basicAuthEndDate)
                            .match(/(\d{4})(\d{2})(\d{2})/)
                            .filter((item, index) => index > 0)
                            .join("-");
                    }
    ...
    render() {
        ...
        const { showDate, visible } = this.state;
        let insertText = "";
        ...
        switch (isExpire) {
                ...
                if (productCode == "HLL_CRM_Marketingbox") {
                    insertText = (
                        <span className={styles.noticeText}>
                            营销大礼包将于
                            <span style={{ color: "#dd576b" }}>
                                {" "}
                                {showDate}{" "}
                            </span>
                            到期，请联系哗啦啦工作人员续费，以免影响使用，谢谢
                        </span>
                    );
                }
                break;
            case "expired": //已过期
                if (productCode == "HLL_CRM_Marketingbox") {
                    insertText = (
                        <span className={styles.noticeText}>
                            授权模块（营销大礼包）已到期，请尽快联系哗啦啦工作人员处理，以免影响使用，谢谢
                        </span>
                    );
                }
                break;
        ...
        return isExpire === "noExpire" || !visible ? null : (
            <Col
                className={styles.expireDateNotice}
            ...
            >
                ...
                <Tooltip title={insertText}>{insertText}</Tooltip>
