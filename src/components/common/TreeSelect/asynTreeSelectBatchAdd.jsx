 ...
 * 功能：树状多选
 * 杨珂   2016/12/09
 * ben 2017-06-02 换名字为 MultiTreeSelect
 */
        ...
        return (
            <div className={styles.SelectedLi}>
                ...
                <div className={styles.SelectedTable}>
                    <ul className={styles.SelectedThead}>
                        <li className={styles.LiStyle}>
                            <span className={styles.spanIndex}>序号</span>
                            <span className={styles.spanCode}>品项编码</span>
                            <span className={styles.spanName}>品项名称</span>
                            <span className={styles.spanDesc}>规格</span>
                            <span className={styles.spanUnit}>单位</span>
                            <span className={styles.spanActive}>状态</span>
                        </li>
                    </ul>
                    <ul className={styles.SelectedTbody}>
                        {this.props.showCities.map((item, index) => {
                            return (
                                <li className={styles.LiStyle} key={index} onClick={() => { this.props.onChange(item) }}>
                                    <span className={styles.SpanIndex}>{index + 1}</span>
                                    <span className={styles.SpanCode} title={item[this.props.itemCode]}>{item[this.props.itemCode]}</span>
                                    <span className={styles.SpanName} title={item[this.props.itemName]}>{item[this.props.itemName]}</span>
                                    <span className={styles.SpanDesc} title={item[this.props.itemDesc]}>{item[this.props.itemDesc]}</span>
                                    <span className={styles.SpanUnit} title={item[this.props.itemUnit]}>{item[this.props.itemUnit]}</span>
                                    <span className={styles.SpanActive}>
                                        {item[this.props.itemActive] == 0 ? '未启用' : '启用'}
                                    </span>
                                </li>)
                        })}
                    </ul>
...
export default class TreeSelect extends Component {
    constructor(props) {
        ...
        // console.log(treeCopy, '--------this.treeCopy--------')
    ...
    handleLevel2Change = (selectedCities) => {
        ...
        // console.log(this.allSelected, '-------this.allSelected--------', index, 'index');
        ...
        // 增加品项去重的方法，当点击不同的类会出现相同品项，要去重
        const selectIDs = [];
        showCities.length && showCities.map((item) => {
            selectIDs.push(item.goodsID);
        })
        const uniqID = uniq(selectIDs);
        const noRepeatData = []
    ...
    render() {
        this.props.getResult(this.state.showCities);
