...
/** 庆丰集团groupID */
export const QINGFENG_GROUP_ID = HUALALA.ENVIRONMENT === 'production-release' ? '-1' : '10890';
/** 护国寺小吃集团groupID */
...
export function isGroupOfHuaTianGroupList(id) {
    if (!id) return false;
    return HUATIAN_GROUP_LIST.includes(String(id));
}

export function isBrandOfHuaTianGroupList(id) {
    if (!id) return false;
    return HUATIAN_BRAND_LIST.includes(String(id));
}

export const BASIC_PROMOTION_CREATE_DISABLED_TIP = '基础营销活动暂时无法创建, 详情请联系管理人员';
export const SPECIAL_PROMOTION_CREATE_DISABLED_TIP = '特色营销活动暂时无法创建, 详情请联系管理人员';
